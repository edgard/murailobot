...
func NewDB(config *Config) (*DB, error) {
	...
	if err != nil {
		return nil, WrapError("failed to connect to database: %w", err)
	}

	db := &DB{conn: conn}
	if err := db.setupSchema(); err != nil {
		return nil, WrapError("failed to set up database schema: %w", err)
	}
	return db, nil
}
...
func (db *DB) setupSchema() error {
	...
	if err != nil {
		return WrapError("failed to execute schema setup: %w", err)
	}
	return nil
}
...
func (db *DB) GetOrCreateUser(userID int64, timeout float64) (User, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			if err != nil {
				return user, WrapError("failed to insert new user: %w", err)
			}
		} else {
			return user, WrapError("failed to fetch user: %w", err)
		}
	}
	return user, nil
}
...
func (db *DB) UpdateUserLastUsed(user User) error {
	...
	if err != nil {
		return WrapError("failed to update user last used time: %w", err)
	}
	return nil
}
...
func (db *DB) GetRandomMessageRef() (MessageRef, error) {
	...
	if err != nil {
		return msgRef, WrapError("failed to retrieve random message reference: %w", err)
	}

	msgRef.LastUsed = time.Now()
	_, err = db.conn.Exec(updateQuery, msgRef.LastUsed, msgRef.ID)
	if err != nil {
		return msgRef, WrapError("failed to update message reference last used time: %w", err)
	}
	return msgRef, nil
}
...
func (db *DB) AddMessageRef(msgRef *MessageRef) error {
	...
	if err != nil {
		return WrapError("failed to add message reference: %w", err)
	}
	return nil
}
...
func (db *DB) GetRecentChatHistory(limit int) ([]ChatHistory, error) {
	...
	if err != nil {
		return nil, WrapError("failed to retrieve recent chat history: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.ID, &entry.UserID, &entry.UserName, &entry.UserMsg, &entry.BotMsg, &entry.LastUsed); err != nil {
			return nil, WrapError("failed to scan chat history: %w", err)
		}
		history = append(history, entry)
	}

	if err := rows.Err(); err != nil {
		return nil, WrapError("rows iteration error: %w", err)
	}
	return history, nil
}
...
func (db *DB) AddChatHistory(history *ChatHistory) error {
	...
	if err != nil {
		return WrapError("failed to add chat history: %w", err)
	}
	return nil
}
