...
// It calls the factory function for each handler (e.g., newHelpHandler).
// Admin commands are wrapped with the AdminOnly middleware.
func RegisterAllCommands(deps HandlerDeps) map[string]RegisteredHandler {
	...
	handlers["/mrl_reset"] = RegisteredHandler{
		...
		MatchType:   tgbot.MatchTypeCommandStartOnly,
		Middleware:  adminMiddleware, // Apply admin check
	}
	...
	// Removed redundant log as Telegram setup will log registration details
	return handlers
}
