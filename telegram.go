...
func (tg *Telegram) handleIncomingMessage(b *gotgbot.Bot, ctx *ext.Context) error {
	if ctx.EffectiveMessage == nil {
		return WrapError(fmt.Errorf("ctx.EffectiveMessage is nil"))
	}
	...
	return WrapError(tg.sendTelegramMessage(ctx, "Mensagem adicionada ao banco de dados!"))
}
...
func (tg *Telegram) handleStartRequest(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	return WrapError(tg.sendTelegramMessage(ctx, "Olá! Me encaminhe uma mensagem para guardar."))
}
...
func (tg *Telegram) handlePiuRequest(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	return WrapError(tg.forwardTelegramMessage(ctx, msgRef.ChatID, msgRef.MessageID))
}
